#!/usr/bin/env python3
"""
load_references.py — Load & pretty-print sample JSON của "đọc hiểu chủ đề" cho AI agent.

Đọc các file `data/doc_hieu_chu_de_n{1,2}_clean.json` (ở project root) và in ra:
  - Stats tổng quan (count, min/max/p25/p50/p75/avg jp_char_count, phân bố Q count/bài)
  - Random samples theo level (để gen agent calibrate style, độ dài, thesis)
  - 1 sample cụ thể theo id

Chỉ có N1 và N2 — các level khác KHÔNG CÓ kind "đọc hiểu chủ đề".
...
def data_path(data_dir: str, level: str) -> str:
    n = level.lower()
    return os.path.join(data_dir, f"doc_hieu_chu_de_{n}_clean.json")
...
def print_stats(data_dir: str) -> None:
    ...
    print("  N1: 1000-1200  |  N2: 900-1100")
    print()
    print("Số câu hỏi CHUẨN (từ rules/question_format.json):")
    print("  N1: 3 câu/bài  |  N2: 3 câu/bài")
    print()
    print("Câu cuối BẮT BUỘC = question_author_opinion hoặc question_content_match.")
    print()
    print("Lưu ý: N3, N4, N5 KHÔNG có kind 'đọc hiểu chủ đề'.")
...
def main():
    ap = argparse.ArgumentParser(description="Load JLPT đọc hiểu chủ đề reference samples (N1, N2 only).")
    ...
    ap.add_argument("--count", type=int, default=1, help="Số sample random (default 1 — bài dài abstract)")
